...
```{r}
DTM <- DocumentTermMatrix(tweets_corpus, control = list(weighting = weightTfIdf))
inspect(DTM)
```
Finally, report the 50 words with the the highest tf.idf scores using a lower frequency bound of .8.\
!!!!!This is not working!!!!
```{r}
#follow page 7 from vignette("tm) to do this by creating a dictionary to restrict dimension of matrix
#create dictionary
top_words <- findFreqTerms(DTM, lowfreq = .8)
#restrict dimension
top_dtm <- DocumentTermMatrix(tweets_corpus, list(dictionary = top_words))
#top 50
top_dtm <- as.matrix(top_dtm) #this is where the error is happening - "error: vector memory exhausted(limit reached?)"
top_dtm %>% 
